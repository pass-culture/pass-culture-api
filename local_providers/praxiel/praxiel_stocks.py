...
class PraxielStocks(LocalProvider):
    ...
    def __next__(self) -> List[ProvidableInfo]:
        try:
            self.praxiel_stock = next(self.praxiel_stock_data)
        except StopIteration:
            self.praxiel_stock_data = api_praxiel_stocks.stocks_information(self.siret,
                                                                      self.last_processed_isbn,
                                                                      self.modified_since)
            self.praxiel_stock = next(self.praxiel_stock_data)

        self.last_processed_isbn = str(self.praxiel_stock['ref'])
        self.product = product_queries.find_active_book_product_by_isbn(self.praxiel_stock['ref'])
        if not self.product:
            return []

        providable_info_offer = self.create_providable_info(OfferSQLEntity,
                                                            f"{self.praxiel_stock['ref']}@{self.venue.siret}",
                                                            datetime.utcnow())
    ...
    def fill_stock_attributes(self, stock: StockSQLEntity):
        ...
        stock.quantity = self.praxiel_stock['available'] + bookings_quantity
        stock.bookingLimitDatetime = None
        stock.offerId = self.offer_id
        stock.price = self.praxiel_stock['price']
