...
from repository.offer_queries import (
    find_searchable_offer,
    get_offers_for_recommendation_v3,
)
from repository.recommendation_queries import (
    ...
    if mediation_id or offer_id:
        ...
        if recommendation is None:
            with db.session.no_autoflush:
                recommendation = _create_recommendation_from_ids(
                    user, offer_id, mediation_id=mediation_id
                )
            repository.save(recommendation)
            logger.debug(
                lambda: "Creating Recommendation with offer_id=%s mediation_id=%s"
...
def create_recommendations_for_discovery(
    ...
    offers = get_offers_for_recommendations_discovery(
        limit=limit, user=user, sent_offers_ids=sent_offers_ids
    )
...
def _get_recommendation_with_information(
    ...
    return (
        ...
        .options(joinedload(Recommendation.offer).joinedload(OfferSQLEntity.stocks))
        .options(joinedload(Recommendation.offer).joinedload(OfferSQLEntity.mediations))
        .all()
    )
...
def _create_recommendation(
    user: UserSQLEntity, offer: OfferSQLEntity, mediation: MediationSQLEntity = None
) -> Recommendation:
