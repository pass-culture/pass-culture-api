...
class ApiLocalProvider():
    ...
    def get_stocks_from_local_provider_api(self, siret: str, last_processed_isbn: str = '', modified_since: str = '', limit: int = 1000) -> Dict:
        ...
        if response.status_code != 200:
            raise ApiLocalProviderException(f'Error {response.status_code} when getting {self.name} stocks for SIRET: {siret}')
