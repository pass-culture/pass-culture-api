...
class CreateBeneficiaryTest:
    ...
    def test_saved_a_beneficiary_from_application(self, stubed_random_token, app):
        # Given
    ...
    @patch("pcapi.connectors.beneficiaries.jouve_backend._get_raw_content", return_value=JOUVE_CONTENT)
    @freeze_time("2013-05-15 09:00:00")
    def test_application_for_native_app_user(self, app):
        # Given
    ...
    def test_application_for_native_app_user_with_load_smoothing(self, _get_raw_content, app, db_session):
        # Given
        application_id = 35
    ...
    @override_features(FORCE_PHONE_VALIDATION=False)
    @patch("pcapi.connectors.beneficiaries.jouve_backend._get_raw_content", return_value=JOUVE_CONTENT)
    def test_cannot_save_beneficiary_if_email_is_already_taken(self, app):
        # Given
        email = "rennes@example.org"
    ...
    @patch("pcapi.connectors.beneficiaries.jouve_backend._get_raw_content", return_value=JOUVE_CONTENT)
    def test_cannot_save_beneficiary_if_duplicate(self, app):
        # Given
        first_name = "Thomas"
        last_name = "DURAND"
    ...
    def test_cannot_save_beneficiary_if_department_is_not_eligible_legacy_behaviour(self, get_application_content, app):
        # Given
        postal_code = "75000"
    ...
    def test_cannot_save_beneficiary_if_department_is_not_eligible(self, get_application_content, app):
        # Given
        postal_code = "984"
        get_application_content.return_value = JOUVE_CONTENT | {"postalCode": postal_code}
    ...
    @patch("pcapi.connectors.beneficiaries.jouve_backend._get_raw_content")
    def test_calls_send_rejection_mail_with_validation_error(self, _get_raw_content, stubed_validate, app):
        # Given
    ...
    def test_id_piece_number_no_duplicate(
        ...
        subscribing_user = users_factories.UserFactory(
            isBeneficiary=False,
    ...
    def test_id_piece_number_duplicate(
        ...
        users_factories.UserFactory(idPieceNumber=ID_PIECE_NUMBER)
        mocked_get_content.return_value = BASE_JOUVE_CONTENT | {"bodyPieceNumber": ID_PIECE_NUMBER}
        ...
        assert beneficiary_import.currentStatus == ImportStatus.REJECTED
        assert (
            beneficiary_import.detail == f"Fraud controls triggered: id piece number n°{ID_PIECE_NUMBER} already taken"
        )
    ...
    def test_id_piece_number_invalid(self, mocked_get_content, wrong_piece_number):
        subscribing_user = users_factories.UserFactory(
            isBeneficiary=False,
            dateOfBirth=datetime.now() - relativedelta(years=18, day=5),
            email=BASE_JOUVE_CONTENT["email"],
        )
        users_factories.UserFactory(idPieceNumber=wrong_piece_number)
    ...
    @patch("pcapi.connectors.beneficiaries.jouve_backend._get_raw_content")
    @pytest.mark.parametrize("wrong_piece_number", ["NOT_APPLICABLE", "KO", ""])
    def test_id_piece_number_wrong_return_control(self, mocked_get_content, wrong_piece_number):
        subscribing_user = users_factories.UserFactory(
            isBeneficiary=False,
            dateOfBirth=datetime.now() - relativedelta(years=18, day=5),
            email=BASE_JOUVE_CONTENT["email"],
    ...
    def test_id_piece_number_wrong_format(self, mocked_get_content, id_piece_number):
        subscribing_user = users_factories.UserFactory(
            isBeneficiary=False,
            dateOfBirth=datetime.now() - relativedelta(years=18, day=5),
            email=BASE_JOUVE_CONTENT["email"],
    ...
    def test_id_piece_number_by_pass(
        ...
        users_factories.UserFactory(idPieceNumber=ID_PIECE_NUMBER)
        users_factories.UserFactory(idPieceNumber=None)
        mocked_get_content.return_value = BASE_JOUVE_CONTENT | {"bodyPieceNumber": ID_PIECE_NUMBER}
    ...
    @patch("pcapi.connectors.beneficiaries.jouve_backend._get_raw_content")
    def test_jouve_raise_403(self, mocked_get_content, caplog):
