...
class CreateRecommendationsForDiscoveryTest:
    ...
    def test_does_not_put_mediation_ids_of_inactive_mediations(self, app):
        ...
        recommendations = create_recommendations_for_discovery(
            sent_offers_ids=sent_offers_ids, user=user
        )
    ...
    @patch(
        "recommendations_engine.recommendations.get_offers_for_recommendations_discovery"
    )
    ...
    def test_returns_offer_in_all_ile_de_france_for_user_from_93(self, app):
        ...
        expected_stocks_recommended = (
            _create_and_save_stock_for_offerer_in_departements(
                offerer_ok, departements_ok
            )
        )
        expected_stocks_not_recommended = (
            _create_and_save_stock_for_offerer_in_departements(
        ...
        repository.save(
            *(expected_stocks_recommended + expected_stocks_not_recommended)
        )
        discovery_view_queries.refresh(concurrently=False)

        offer_ids_in_adjacent_department = set(
            [stock.offerId for stock in expected_stocks_recommended]
        )

        #  when
        recommendations = create_recommendations_for_discovery(
            sent_offers_ids=sent_offers_ids, limit=10, user=user
        )

        # then
        recommended_offer_ids = set(
            [recommendation.offerId for recommendation in recommendations]
        )
    ...
    def test_returns_offers_from_any_departement_for_user_from_00(self, app):
        ...
        offer_ids_in_all_department = set(
            [stock.offerId for stock in expected_stocks_recommended]
        )

        #  when
        recommendations = create_recommendations_for_discovery(
            limit=10, sent_offers_ids=sent_offers_ids, user=user
        )
...
class GiveRequestedRecommendationToUserTest:
    ...
    @clean_database
    def test_when_recommendation_exists_for_other_user_returns_a_new_one_for_the_current_user(
        self, app
    ):
        # Given
        user = create_user()
...
def _create_and_save_stock_for_offerer_in_departements(
    ...
    for index, departement_code in enumerate(departement_codes):
        ...
        venue = create_venue(
            offerer, postal_code="{:5}".format(departement_code), siret=siret
        )
