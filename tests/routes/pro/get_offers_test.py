...
class Returns200Test:
    ...
    @patch("pcapi.routes.pro.offers.offers_api.list_offers_for_pro_user")
    def should_filter_offers_by_given_subcategory_id(self, mocked_list_offers, app, db_session):
        # given
        pro = users_factories.ProFactory()
        offerer = offers_factories.OffererFactory()
        offers_factories.UserOffererFactory(user=pro, offerer=offerer)
        ...
        # when
        response = (
            TestClient(app.test_client()).with_session_auth(email=pro.email).get("/offers?subcategoryId=LIVRE_PAPIER")
        )
