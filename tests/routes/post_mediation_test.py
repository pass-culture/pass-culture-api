...
class Post:
    class Returns201:
        ...
        def when_mediation_is_created_with_thumb_url(
            self, read_thumb, mock_redis, mock_feature, app
        ):
            # given
            user = create_user()
        ...
        @patch("routes.mediations.feature_queries.is_active", return_value=True)
        @patch("routes.mediations.redis.add_offer_id")
        def when_mediation_is_created_with_thumb_file(
        ...
        @patch("routes.mediations.feature_queries.is_active", return_value=True)
        @patch("routes.mediations.redis.add_offer_id")
        def should_add_offer_id_to_redis_when_mediation_is_created_with_thumb(
            self, mock_redis, mock_feature, app
        ):
            # given
            user = create_user()
    ...
    class Returns400:
        ...
        def when_mediation_is_created_with_thumb_url_pointing_to_not_an_image(
            self, mock_thumb_storage_request, app
        ):
            # given
            api_response = {}
            response_return_value = MagicMock(status_code=200, text="")
            response_return_value.headers = MagicMock(
