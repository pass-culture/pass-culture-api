...
class AllocineStocksTest:
    class InitTest:
        ...
        def test_should_call_allocine_api(self, mock_call_allocine_api, app):
            ...
            venue = create_venue(
                offerer, name="Cinéma Allociné", siret="77567146400110"
            )
    ...
    class NextTest:
        ...
        @clean_database
        def test_should_return_providable_infos_for_each_movie(
            self, mock_call_allocine_api, app
        ):
            # Given
            ...
            assert product_providable_info.date_modified_at_provider == datetime(
                year=2019, month=10, day=15, hour=9
            )

            assert offer_providable_info.type == OfferSQLEntity
            assert (
                offer_providable_info.id_at_providers
                == "TW92aWU6Mzc4MzI=%77567146400110-VF"
            )
            assert offer_providable_info.date_modified_at_provider == datetime(
                year=2019, month=10, day=15, hour=9
            )

            assert stock_providable_info.type == StockSQLEntity
            assert (
                stock_providable_info.id_at_providers
                == "TW92aWU6Mzc4MzI=%77567146400110#DUBBED/2019-10-29T10:30:00"
            )
            assert stock_providable_info.date_modified_at_provider == datetime(
                year=2019, month=10, day=15, hour=9
            )
...
class UpdateObjectsTest:
    @patch(
        "local_providers.local_provider.feature_queries.is_active", return_value=True
    )
    ...
    def test_should_create_one_product_and_one_original_version_offer_and_one_dubbed_version_offer_with_movie_info(
        ...
        assert (
            original_version_offer.description
            == "synopsis du film\nTous les détails du film sur AlloCiné:"
            " http://www.allocine.fr/film/fichefilm_gen_cfilm=37832.html"
        )
        ...
        assert (
            dubbed_version_offer.description
            == "synopsis du film\nTous les détails du film sur AlloCiné:"
            " http://www.allocine.fr/film/fichefilm_gen_cfilm=37832.html"
        )
    ...
    def test_should_update_existing_product_duration_and_create_new_offer_when_no_offer_exists(
        ...
        repository.save(
            venue, product, allocine_venue_provider, venue_provider_price_rule
        )
    ...
    def test_should_create_product_with_correct_thumb_and_increase_thumbCount_by_1(
        ...
        assert (
            existing_product.thumbUrl
            == f"http://localhost/storage/thumbs/products/{humanize(existing_product.id)}"
        )
    ...
    def test_should_create_one_product_and_one_offer_and_associated_stocks(
        ...
        vf_offer = OfferSQLEntity.query.filter(OfferSQLEntity.name.contains("VF")).one()
    ...
    class WhenAllocineStockAreSynchronizedTwice:
        ...
        @patch.dict("os.environ", {"ALLOCINE_API_KEY": "token"})
        @clean_database
        def test_should_update_stocks_based_on_stock_date(
            ...
            # Then
            created_stock = StockSQLEntity.query.order_by(
        ...
        @patch(
            "local_providers.local_provider.feature_queries.is_active",
            return_value=True,
        )
        @patch("local_providers.local_provider.send_venue_provider_data_to_redis")
        @patch("local_providers.allocine.allocine_stocks.get_movies_showtimes")
        ...
        def test_should_create_one_different_offer_and_stock_for_different_venues(
            ...
            venue_provider_price_rule1 = create_allocine_venue_provider_price_rule(
                venue_provider1
            )
            ...
            repository.save(
                venue_provider1,
                venue_provider2,
                venue_provider_price_rule1,
                venue_provider_price_rule2,
            )
            ...
            assert len(created_offer) == 2
            assert (
                OfferSQLEntity.query.filter(OfferSQLEntity.venueId == venue1.id).count()
                == 1
            )
            assert (
                OfferSQLEntity.query.filter(OfferSQLEntity.venueId == venue2.id).count()
                == 1
            )
            assert len(created_stock) == 2
    ...
    class WhenOfferHasBeenManuallyUpdated:
        ...
        @patch.dict("os.environ", {"ALLOCINE_API_KEY": "token"})
        @clean_database
        def test_should_preserve_manual_modification(
            self, mock_poster_get_allocine, mock_call_allocine_api, app
        ):
            # Given
            theater_token = "test"
    ...
    class WhenStockHasBeenManuallyDeleted:
        ...
        @patch.dict("os.environ", {"ALLOCINE_API_KEY": "token"})
        @clean_database
        def test_should_preserve_deletion(
            self, mock_poster_get_allocine, mock_call_allocine_api, app
        ):
            # Given
            theater_token = "test"
            ...
            offerer = create_offerer(siren="775671464")
            venue = create_venue(
                offerer, name="Cinema Allocine", siret="77567146400110"
            )
            repository.save(venue)
    ...
    class WhenSettingDefaultValuesAtImport:
        ...
        @patch.dict("os.environ", {"ALLOCINE_API_KEY": "token"})
        @clean_database
        def test_should_preserve_is_duo_default_value(
            self, mock_poster_get_allocine, mock_call_allocine_api, app
        ):
            # Given
            theater_token = "test"
            ...
            allocine_provider = activate_provider("AllocineStocks")
            allocine_venue_provider = create_allocine_venue_provider(
                venue, allocine_provider, is_duo=True
            )
            venue_provider_price_rule = create_allocine_venue_provider_price_rule(
                allocine_venue_provider
            )
        ...
        @patch.dict("os.environ", {"ALLOCINE_API_KEY": "token"})
        @clean_database
        def test_should_preserve_quantity_default_value(
            self, mock_poster_get_allocine, mock_call_allocine_api, app
        ):
            # Given
            theater_token = "test"
            ...
            allocine_provider = activate_provider("AllocineStocks")
            allocine_venue_provider = create_allocine_venue_provider(
                venue, allocine_provider, quantity=50
            )
            venue_provider_price_rule = create_allocine_venue_provider_price_rule(
                allocine_venue_provider
            )
...
class GetObjectThumbTest:
    ...
    @patch.dict("os.environ", {"ALLOCINE_API_KEY": "token"})
    @clean_database
    def test_should_get_movie_poster_if_poster_url_exist(
        self, mock_poster_get_allocine, mock_call_allocine_api, app
    ):
        # Given
