...
class LibrairesStocksTest:
    class NextTest:
        ...
        def test_should_return_providable_infos_with_correct_data(self, mock_libraires_api_response, app):
            # Given
            mock_libraires_api_response.return_value = iter([{"ref": "9780199536986", "available": 1, "price": 6.36}])
    ...
    class UpdateObjectsTest:
        ...
        @pytest.mark.usefixtures("db_session")
        @patch("pcapi.local_providers.libraires.libraires_stocks.api_libraires_stocks.stocks_information")
        def test_stock_provider_libraires_create_one_stock_and_one_offer_with_wanted_attributes(
            self, mock_libraires_api_response, app
        ):
            # Given
            mock_libraires_api_response.return_value = iter([{"ref": "9780199536986", "available": 10, "price": 16.5}])
            ...
            venue_provider = create_venue_provider(
                venue, libraires_stocks_provider, is_active=True, venue_id_at_offer_provider="77567146400110"
            )
            product = create_product_with_thing_type(id_at_providers="9780199536986")
        ...
        @pytest.mark.usefixtures("db_session")
        @patch("pcapi.local_providers.libraires.libraires_stocks.api_libraires_stocks.stocks_information")
        def test_stock_provider_libraires_update_one_stock_and_update_matching_offer(
            self, mock_libraires_api_response, app
        ):
            # Given
            mock_libraires_api_response.return_value = iter([{"ref": "9780199536986", "available": 10, "price": 16}])
            ...
            libraires_stocks_provider = activate_provider("LibrairesStocks")
            venue_provider = create_venue_provider(
                venue, libraires_stocks_provider, is_active=True, venue_id_at_offer_provider="12345678912345"
            )
            product = create_product_with_thing_type(id_at_providers="9780199536986")
        ...
        @pytest.mark.usefixtures("db_session")
        @patch("pcapi.local_providers.libraires.libraires_stocks.api_libraires_stocks.stocks_information")
        def test_libraires_stocks_create_2_stocks_and_2_offers_even_if_existing_offer_on_same_product(
            self, mock_libraires_api_response, app
        ):
            # Given
            ...
            offer = create_offer_with_thing_product(
                venue, product=product_2, id_at_providers="everything_but_libraires"
            )
        ...
        @pytest.mark.usefixtures("db_session")
        @patch("pcapi.local_providers.libraires.libraires_stocks.api_libraires_stocks.stocks_information")
        def test_stock_provider_libraires_available_stock_is_sum_of_updated_available_and_bookings(
            self, mock_libraires_api_response, app
        ):
            # Given
            mock_libraires_api_response.return_value = iter([{"ref": "9780199536986", "available": 5, "price": 0}])
            ...
            mock_libraires_api_response.return_value = iter([{"ref": "9780199536986", "available": 66, "price": 0}])
