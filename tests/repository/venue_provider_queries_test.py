...
from repository.venue_provider_queries import get_venue_providers_to_sync, get_nb_containers_at_work, \
    get_actives_venue_providers_for_specific_provider
...
class GetVenueProvidersToSyncTest:
    ...
    @clean_database
    def test_should_return_only_active_venue_provider(self, app):
        # Given
        offerer = create_offerer()
        venue_1 = create_venue(offerer, siret='12345678901234')
        venue_2 = create_venue(offerer)
        titelive_provider = activate_provider('TiteLiveStocks')
        venue_provider_1 = create_venue_provider(venue_1, titelive_provider, is_active=True, sync_worker_id=None)
        venue_provider_2 = create_venue_provider(venue_2, titelive_provider, is_active=False, sync_worker_id=None)
        repository.save(venue_provider_1, venue_provider_2)
...
class GetNbContainersAtWorkTest:
    ...
    def test_should_return_number_of_venue_provider_with_worker_id(self, app):
        ...
        titelive_provider = activate_provider('TiteLiveStocks')
        venue_provider_1 = create_venue_provider(venue1, titelive_provider)
        venue_provider_2 = create_venue_provider(venue2, titelive_provider, sync_worker_id='1234567')
        repository.save(venue_provider_1, venue_provider_2)
