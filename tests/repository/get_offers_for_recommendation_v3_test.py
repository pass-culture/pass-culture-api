    ...
    class UniversalBehaviorTest:
        class FiltersTest:
            ...
            def test_filter_should_not_return_activation_event(self, app):
                ...
                offer_activation = create_offer_with_event_product(
                ...
                offers = get_offers_for_recommendation_v3(
                    user=user, user_is_geolocated=False
                )
            ...
            def test_filter_should_return_offers_with_mediation_only(self, app):
                ...
                user = create_user()
                venue = create_venue(
                    offerer, postal_code="34000", departement_code="34"
                )
            ...
            @clean_database
            def test_filter_should_not_return_offers_having_only_soft_deleted_stocks(
                self, app
            ):
                # Given
                product = create_product_with_thing_type(
            ...
            @clean_database
            def test_filter_should_not_return_offers_from_non_validated_venue(
                self, app
            ):
                # Given
                product = create_product_with_thing_type(
            ...
            @clean_database
            def test_filter_should_not_return_offers_from_non_validated_offerers(
                self, app
            ):
                # Given
                product = create_product_with_thing_type(
            ...
            @clean_database
            def test_filter_should_not_return_offers_having_only_stocks_with_past_booking_limit_date_time(
                self, app
            ):
                # Given
                product = create_product_with_thing_type(
                ...
                stock = create_stock_from_offer(
                    offer, quantity=2, booking_limit_datetime=one_day_ago
                )
                create_mediation(stock.offer)
        ...
        class OrderTest:
            @clean_database
            def test_order_with_criteria_should_return_offer_with_highest_base_score_first(
                self, app
            ):
                # Given
                offerer = create_offerer()
                ...
                # When
                offers = get_offers_for_recommendation_v3(
                    user=user, user_is_geolocated=False, limit=1
                )
            ...
            @clean_database
            @patch(
                "repository.offer_queries.feature_queries.is_active", return_value=True
            )
            def test_order_should_return_ordered_offers_by_dateSeen_when_feature_is_active(
                self, app
            ):
                # Given
                offerer = create_offerer()
                ...
                seen_offer_1 = create_seen_offer(
                    offer_1, user, date_seen=datetime.utcnow() - timedelta(hours=12)
                )
                seen_offer_2 = create_seen_offer(
                    offer_2, user, date_seen=datetime.utcnow()
                )

                repository.save(
                    user,
                    stock_digital_offer_1,
                    stock_physical_offer_2,
                    seen_offer_1,
                    seen_offer_2,
                )
            ...
            def test_order_should_return_unseen_offers_first_when_feature_is_active(
                self, app
            ):
                # Given
                offerer = create_offerer()
                ...
                offer_1 = create_offer_with_thing_product(
                    venue=venue, is_national=True, thing_type=ThingType.LIVRE_EDITION
                )
                offer_2 = create_offer_with_thing_product(
                    venue=venue, is_national=True, thing_type=ThingType.LIVRE_EDITION
                )
                ...
                seen_offer = create_seen_offer(
                    offer_2, user, date_seen=datetime.utcnow()
                )

                repository.save(
                    user, stock_digital_offer_1, stock_physical_offer_2, seen_offer
                )
            ...
            @clean_database
            @patch(
                "repository.offer_queries.feature_queries.is_active", return_value=False
            )
            @patch("repository.offer_queries.order_offers_by_unseen_offers_first")
            def test_order_should_not_order_offers_by_dateSeen_when_feature_is_not_active(
    ...
    class UserSpecificBehaviorTest:
        ...
        class WhenUserIsGeolocatedTest:
            ...
            def test_filter_should_return_offer_when_offer_is_national(self, app):
                ...
                polygon2 = Polygon(
                    [(2.195693, 50), (2.195693, 48), (2.595697, 48), (2.595697, 50)]
                )
                ...
                offers = get_offers_for_recommendation_v3(
                    user=user,
                    user_iris_id=user_location_iris.id,
            ...
            @clean_database
            def test_filter_should_not_return_offers_from_venue_outside_user_iris(
                self, app
            ):
                # given
                offerer = create_offerer(siren="123456789")
            ...
            @clean_database
            def test_filter_should_return_only_national_offers_when_user_is_geolocated_abroad(
                self, app
            ):
                # given
                offerer = create_offerer(siren="123456789")
                ...
                national_offer = create_offer_with_thing_product(
                    venue, is_national=True
                )
                stock = create_stock_from_offer(offer)
                ...
                repository.save(
                    user, stock, stock_on_digital_offer, stock_on_national_offer
                )
                ...
                # when
                offers = get_offers_for_recommendation_v3(
                    user=user, user_iris_id=None, user_is_geolocated=True
                )
