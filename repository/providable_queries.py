...
def update_chunk(chunk_to_update: Dict):
    models_in_chunk = set(
        _extract_model_name_from_chunk_key(key) for key in chunk_to_update.keys()
    )

    for model_in_chunk in models_in_chunk:
        matching_tuples_in_chunk = _filter_matching_pc_object_in_chunk(
            model_in_chunk, chunk_to_update
        )
        values_to_update_in_chunk = _extract_dict_values_from_chunk(
            matching_tuples_in_chunk
        )
...
def _filter_matching_pc_object_in_chunk(
    model_in_chunk: Model, chunk_to_update: Dict
) -> List[Model]:
    return list(
        filter(
            lambda item: _extract_model_name_from_chunk_key(item[0]) == model_in_chunk,
            chunk_to_update.items(),
        )
    )


def _extract_dict_values_from_chunk(
    matching_tuples_in_chunk: List[Model],
) -> List[Dict]:
    return list(
        dictify_pc_object(pc_object_item)
        for pc_object_key, pc_object_item in matching_tuples_in_chunk
    )


def get_existing_object(model_type: Model, id_at_providers: str) -> Optional[Dict]:
    query = select([model_type]).where(model_type.idAtProviders == id_at_providers)
    db_object_dict = db.session.execute(query).fetchone()
...
def get_last_update_for_provider(provider_id: int, pc_obj: Model) -> datetime:
    if pc_obj.lastProviderId == provider_id:
        return (
            pc_obj.dateModifiedAtLastProvider
            if pc_obj.dateModifiedAtLastProvider
            else None
        )
    return None
