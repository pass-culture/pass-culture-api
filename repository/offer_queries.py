...
def department_or_national_offers(query, departement_codes):
    if ALL_DEPARTMENTS_CODE in departement_codes:
        return query.filter(
            (VenueSQLEntity.departementCode != None)
            | (OfferSQLEntity.isNational == True)
        )

    query = query.filter(
        VenueSQLEntity.departementCode.in_(departement_codes)
        | (OfferSQLEntity.isNational == True)
    )
...
def _bookable_offers(offers_query: Query) -> Query:
    stocks_query = StockSQLEntity.query.filter(
        StockSQLEntity.offerId == OfferSQLEntity.id
    )
...
def _with_active_and_validated_offerer(query):
    ...
    logger.debug(
        lambda: "(reco) from active and validated offerer .count" + str(query.count())
    )
    return query
...
def get_offers_for_recommendation(
    ...
    if ALL_DEPARTMENTS_CODE not in departement_codes:
        ...
        discovery_view_query = keep_only_offers_in_venues_or_national(
            discovery_view_query, venue_ids
        )

    if feature_queries.is_active(FeatureToggle.SAVE_SEEN_OFFERS):
        discovery_view_query = order_offers_by_unseen_offers_first(
            discovery_view_query, DiscoveryView, user
        )

    discovery_view_query = discovery_view_query.order_by(
        DiscoveryView.offerDiscoveryOrder
    )
...
def get_offers_for_recommendation_v3(
    ...
    if user_is_geolocated:
        ...
        discovery_view_query = (
            keep_only_offers_from_venues_located_near_to_user_or_national(
                discovery_view_query, venue_ids
            )
        )
    ...
    if feature_queries.is_active(FeatureToggle.SAVE_SEEN_OFFERS):
        discovery_view_query = order_offers_by_unseen_offers_first(
            discovery_view_query, DiscoveryViewV3, user
        )

    discovery_view_query = discovery_view_query.order_by(
        DiscoveryViewV3.offerDiscoveryOrder
    )
...
def order_offers_by_unseen_offers_first(
    query: BaseQuery, discovery_view_model: Model, user: UserSQLEntity
):
    return query.outerjoin(
        SeenOffer,
        (SeenOffer.offerId == discovery_view_model.id) & (SeenOffer.userId == user.id),
...
def keep_only_offers_from_venues_located_near_to_user_or_national(
    ...
    return query.filter(
        or_(DiscoveryViewV3.venueId.in_(venue_ids), DiscoveryViewV3.isNational == True)
    )


def keep_only_offers_in_venues_or_national(
    query: BaseQuery, venue_ids: selectable.Alias
) -> BaseQuery:
    return query.filter(
        or_(DiscoveryView.venueId.in_(venue_ids), DiscoveryView.isNational == True)
    )
...
def get_active_offers_ids_query(
    user, departement_codes=[ALL_DEPARTMENTS_CODE], offer_id=None
):
    ...
    logger.debug(
        lambda: "(reco) all offers count {}".format(active_offers_query.count())
    )
    active_offers_query = active_offers_query.filter_by(isActive=True)
    logger.debug(
        lambda: "(reco) active offers count {}".format(active_offers_query.count())
    )
    active_offers_query = _with_validated_venue(active_offers_query)
    logger.debug(
        lambda: "(reco) offers with venue count {}".format(active_offers_query.count())
    )
    active_offers_query = _with_image(active_offers_query)
    logger.debug(
        lambda: "(reco) offers with image count {} ".format(active_offers_query.count())
    )
    active_offers_query = department_or_national_offers(
        active_offers_query, departement_codes
    )
    logger.debug(
        lambda: "(reco) department or national {} in {}".format(
            str(departement_codes), active_offers_query.count()
        )
    )
    active_offers_query = _bookable_offers(active_offers_query)
    logger.debug(
        lambda: "(reco) bookable_offers {}".format(active_offers_query.count())
    )
    active_offers_query = _with_active_and_validated_offerer(active_offers_query)
    logger.debug(
        lambda: "(reco) active and validated {}".format(active_offers_query.count())
    )
    active_offers_query = _not_activation_offers(active_offers_query)
    logger.debug(
        lambda: "(reco) not_currently_recommended and not_activation {}".format(
...
def _exclude_booked_and_favorite(active_offers_query, user):
    booked_offer_ids = (
        BookingSQLEntity.query.filter_by(userId=user.id)
        .join(StockSQLEntity)
        .with_entities('stock."offerId"')
        .subquery()
    )
    favorite_offer_ids = (
        FavoriteSQLEntity.query.filter_by(userId=user.id)
        .with_entities('"offerId"')
        .subquery()
    )
    ...
    active_offers_query = active_offers_query.filter(
        not_booked_predicate & not_favorite_predicate
    )
...
def _round_robin_by_type_onlineness_and_criteria(order_by: List):
    return func.row_number().over(
        partition_by=[OfferSQLEntity.type, OfferSQLEntity.url == None],
        order_by=order_by(),
    )
...
def _with_validated_venue(offer_query):
    return offer_query.join(
        VenueSQLEntity, OfferSQLEntity.venueId == VenueSQLEntity.id
    ).filter(VenueSQLEntity.validationToken == None)


def _build_occurs_soon_or_is_thing_predicate():
    return StockSQLEntity.query.filter(
        (StockSQLEntity.offerId == OfferSQLEntity.id)
        & (
...
def _build_has_active_mediation_predicate():
    return MediationSQLEntity.query.filter(
        (MediationSQLEntity.offerId == OfferSQLEntity.id) & MediationSQLEntity.isActive
    ).exists()
...
def _offer_has_stocks_compatible_with_days_intervals(days_intervals):
    event_beginningdate_in_interval_filter = or_(
        *map(_date_interval_to_filter, days_intervals)
    )
    stock_has_no_beginning_date_time = StockSQLEntity.beginningDatetime == None
    return (
        StockSQLEntity.query.filter(StockSQLEntity.offerId == OfferSQLEntity.id)
        .filter(
            event_beginningdate_in_interval_filter | stock_has_no_beginning_date_time
        )
        .exists()
    )
...
def find_searchable_offer(offer_id):
    return (
        OfferSQLEntity.query.filter_by(id=offer_id)
        .join(VenueSQLEntity)
        .filter(VenueSQLEntity.validationToken == None)
        .first()
    )
...
def _offer_has_bookable_stocks():
    ...
    event_has_not_began_yet = (StockSQLEntity.beginningDatetime != None) & (
        StockSQLEntity.beginningDatetime > now
    )
        ...
        .filter(
            ...
            | (
                (
                    ...
                    - func.coalesce(bookings_quantity.c.quantity, 0)
                )
                > 0
            )
        )
...
def _build_bookings_quantity_subquery():
    ...
    bookings_quantity = (
        BookingSQLEntity.query.join(stock_alias)
        .filter(BookingSQLEntity.isCancelled == False)
        .group_by(BookingSQLEntity.stockId)
        .with_entities(
            func.sum(BookingSQLEntity.quantity).label("quantity"),
...
def find_activation_offers(departement_code: str) -> List[OfferSQLEntity]:
    departement_codes = (
        ILE_DE_FRANCE_DEPT_CODES if departement_code == "93" else [departement_code]
    )
    ...
    query = (
        OfferSQLEntity.query.join(VenueSQLEntity)
...
def _filter_bookable_stocks_for_discovery(stocks_query):
    ...
    stocks_query = stocks_query.outerjoin(
        bookings_quantity, StockSQLEntity.id == bookings_quantity.c.stockId
    ).filter(
...
def count_offers_for_things_only_by_venue_id(venue_id):
    offer_count = (
        OfferSQLEntity.query.filter_by(venueId=venue_id)
...
def get_offers_by_venue_id(venue_id: int) -> List[OfferSQLEntity]:
    return OfferSQLEntity.query.filter_by(venueId=venue_id).all()


def get_offers_by_product_id(product_id: int) -> List[OfferSQLEntity]:
    return OfferSQLEntity.query.filter_by(productId=product_id).all()


def get_offers_by_ids(offer_ids: List[int]) -> List[OfferSQLEntity]:
    return (
        OfferSQLEntity.query.filter(OfferSQLEntity.id.in_(offer_ids))
...
def get_paginated_active_offer_ids(limit: int, page: int) -> List[tuple]:
    return (
        OfferSQLEntity.query.with_entities(OfferSQLEntity.id)
        .filter(OfferSQLEntity.isActive == True)
...
def get_paginated_offer_ids_by_venue_id(
    venue_id: int, limit: int, page: int
) -> List[tuple]:
    return (
        OfferSQLEntity.query.with_entities(OfferSQLEntity.id)
...
def get_paginated_expired_offer_ids(limit: int, page: int) -> List[tuple]:
    ...
    return (
        OfferSQLEntity.query.join(StockSQLEntity)
