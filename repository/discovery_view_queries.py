...
def order_by_digital_offers() -> str:
    return f"""
        ROW_NUMBER() OVER (
            ORDER BY
                (
...
def clean(app: Flask) -> None:
    db.session.execute(
        "UPDATE feature SET \"isActive\" = FALSE WHERE name = 'UPDATE_DISCOVERY_VIEW'"
    )
    db.session.commit()
    ...
    db.session.execute(
        "UPDATE feature SET \"isActive\" = TRUE WHERE name = 'UPDATE_DISCOVERY_VIEW'"
    )
    db.session.commit()
...
def _create_discovery_view(session: Session, order: Callable, name: str) -> None:
    get_recommendable_offers = (
        _create_function_get_recommendable_offers_ordered_by_digital_offers(
            session, order
        )
    )
    session.execute(
...
def _create_function_get_recommendable_offers_ordered_by_digital_offers(
    session, order: Callable
) -> str:
...
def _create_function_get_active_offers_ids(session) -> str:
    ...
    offer_has_at_least_one_active_mediation = (
        _create_function_offer_has_at_least_one_active_mediation(session)
    )
    offer_has_at_least_one_bookable_stock = (
        _create_function_offer_has_at_least_one_bookable_stock(session)
    )
