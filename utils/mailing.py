...
def add_contact_informations(
    email: str, date_of_birth: str, department_code: str
) -> Response:
    data = {
        "Data": [
...
def make_offerer_driven_cancellation_email_for_offerer(
    ...
    return {
        ...
        "FromEmail": SUPPORT_EMAIL_ADDRESS
        if feature_send_mail_to_users_enabled()
        else DEV_EMAIL_ADDRESS,
        "Subject": email_subject,
        "Html-part": email_html,
    }


def make_user_validation_email(
    user: UserSQLEntity, app_origin_url: str, is_webapp: bool
) -> Dict:
    if is_webapp:
...
def subscribe_newsletter(user: UserSQLEntity):
    ...
    contact_lists_data = {
        "ContactsLists": [{"Action": "addnoforce", "ListID": 1795144}]
    }

    return app.mailjet_client.contact_managecontactslists.create(
        id=contact["ID"], data=contact_lists_data
    ).json()
...
def make_payment_message_email(xml: str, checksum: bytes) -> Dict:
    ...
    return {
        ...
        "Subject": "Virements XML pass Culture Pro - {}".format(
            datetime.strftime(now, "%Y-%m-%d")
        ),
        "Attachments": [
            {"ContentType": "text/xml", "Filename": file_name, "Content": xml_b64encode}
        ],
        "Html-part": render_template(
            "mails/payments_xml_email.html",
            file_name=file_name,
...
def make_payment_details_email(csv: str) -> Dict:
    ...
    return {
        "FromEmail": SUPPORT_EMAIL_ADDRESS,
        "FromName": "pass Culture Pro",
        "Subject": "Détails des paiements pass Culture Pro - {}".format(
            datetime.strftime(now, "%Y-%m-%d")
        ),
        "Attachments": [
            {
                "ContentType": "text/csv",
                "Filename": "details_des_paiements_{}.csv".format(
                    datetime.strftime(now, "%Y%m%d")
                ),
...
def make_payments_report_email(
    not_processable_csv: str, error_csv: str, grouped_payments: Dict
) -> Dict:
    now = datetime.utcnow()
    not_processable_csv_b64encode = base64.b64encode(
        not_processable_csv.encode("utf-8")
    ).decode()
    ...
    return {
        "Subject": "Récapitulatif des paiements pass Culture Pro - {}".format(
            formatted_date
        ),
        "FromEmail": SUPPORT_EMAIL_ADDRESS,
        "FromName": "pass Culture Pro",
...
def make_wallet_balances_email(csv: str) -> Dict:
    ...
    return {
        "FromEmail": SUPPORT_EMAIL_ADDRESS,
        "FromName": "pass Culture Pro",
        "Subject": "Soldes des utilisateurs pass Culture - {}".format(
            datetime.strftime(now, "%Y-%m-%d")
        ),
        "Attachments": [
            {
                "ContentType": "text/csv",
                "Filename": "soldes_des_utilisateurs_{}.csv".format(
                    datetime.strftime(now, "%Y%m%d")
                ),
                "Content": csv_b64encode,
            }
...
def make_activation_users_email(csv: str) -> Dict:
    ...
    return {
        ...
        "Attachments": [
            {
                "ContentType": "text/csv",
                "Filename": "liste_des_utilisateurs_{}.csv".format(
                    datetime.strftime(now, "%Y%m%d")
                ),
                "Content": csv_b64encode,
            }
...
def compute_email_html_part_and_recipients(
    email_html_part, recipients: Union[List[str], str]
) -> (str, str):
...
def make_offer_creation_notification_email(
    offer: OfferSQLEntity, author: UserSQLEntity, pro_origin_url: str
) -> Dict:
