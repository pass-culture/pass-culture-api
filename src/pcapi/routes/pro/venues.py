...
@private_api.route("/venues/<venue_id>/accessibility", methods=["PUT"])
@login_required
@spectree_serialize(on_success_status=204, on_error_statuses=[400])
def edit_venue_accessibility(venue_id: str, body: venues_serialize.EditVenueAccessibilityBodyModel) -> None:
    venue = load_or_404(Venue, venue_id)

    check_user_has_access_to_offerer(current_user, venue.managingOffererId)
    offerers_api.update_venue_accessibillity(venue, body)
