...
class BeneficiaryUserAdminView(BaseAdminView):
    ...
    def get_query(self) -> query:
        return UserSQLEntity.query.outerjoin(UserOfferer).filter(
            UserOfferer.userId == None  # pylint: disable=singleton-comparison
        )
    ...
    def get_count_query(self) -> query:
        return (
            ...
            .filter(UserOfferer.userId == None)  # pylint: disable=singleton-comparison
        )
